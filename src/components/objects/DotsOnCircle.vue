...
import { computed, nextTick, onUpdated, ref, type ComputedRef } from "vue";
...
const solidCx = computed(() => Number(getParameterValue(props.params, "offsetX")) + halfClockSizeX.value);
const solidCy = computed(() => Number(getParameterValue(props.params, "offsetY")) + halfClockSizeY.value);
...
const fillCx = computed(() => halfClockSizeX.value + Number(getParameterValue(computed(() => props.params).value, "offsetX")) + (Number(getParameterValue(props.params, "size")) / 2) * Math.cos(Math.PI * 2 * getNormalTimeValue(getParameterValue(props.params, "relatedTime"), storeTime.time as DateTime) - Math.PI / 2));
const fillCy = computed(() => halfClockSizeY.value + Number(getParameterValue(computed(() => props.params).value, "offsetY")) + (Number(getParameterValue(props.params, "size")) / 2) * Math.sin(Math.PI * 2 * getNormalTimeValue(getParameterValue(props.params, "relatedTime"), storeTime.time as DateTime) - Math.PI / 2));
</script>
